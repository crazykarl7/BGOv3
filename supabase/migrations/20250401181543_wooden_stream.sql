...
USING (
  ...
  OR
  -- Allow players to view teams in their olympics
  EXISTS (
...
TO authenticated
WITH CHECK (
  -- Allow admins to create teams without restrictions
  EXISTS (
    SELECT 1 FROM profiles
    WHERE profiles.id = auth.uid()
    AND profiles.is_admin = true
  ...
  OR
  -- Regular users must be registered for the olympic and not in another team
  (
    EXISTS (
      SELECT 1 FROM olympic_player
...
USING (
  ...
  OR
  -- Allow players to view team members in their olympics
  EXISTS (
    SELECT 1 FROM teams
...
WITH CHECK (
  ...
  OR
  -- Regular users must follow the rules
  (
    EXISTS (
      SELECT 1 FROM teams
      JOIN olympic_player ON olympic_player.olympic_id = teams.olympic_id
...
USING (
  ...
  OR
  -- Allow team members and creators to view assignments
  EXISTS (
    SELECT 1 FROM teams
...
USING (
  ...
  OR
  -- Allow team creators to manage assignments
  (
    EXISTS (
      SELECT 1 FROM teams
      WHERE teams.id = team_id
